...
namespace DMT.Library.Environment
{
	public class Monitors : List<Monitor>
	{
		...
		/// <summary>
		/// Determines if the point is on the primary screen or any in the primary group if specified
		/// 
		/// This will be called from the mouse hook so needs to be efficient
		/// </summary>
		/// <param name="point"></param>
		/// <param name="primaryGroup">If not null, List of monitor indexes (zero based) that are in the primary group.
		/// If null, then we just check the primary monitor</param>
		/// <returns>true if the point is on the primary monitor/group</returns>
		public bool IsOnPrimary(int x, int y, List<int> primaryGroup = null)
		{
			if (primaryGroup == null || primaryGroup.Count == 0)
			{
				// primary group not being used, so we check the primary monitor
				for (int i = 0; i < Count; i++)
				{
					if (this[i].Primary)
					{
						return this[i].Bounds.Contains(x, y);
					}
				}
			}
			else
			{
				// we need to check if the point is on any monitor in the primary group
				// alt: find monitor point is on and then see if that is in primary group
				// - but think this should be faster
				foreach (int i in primaryGroup)
				{
					if (i >= 0 && i < Count)
					{
						if (this[i].Bounds.Contains(x, y))
						{
							return true;
						}
					}
