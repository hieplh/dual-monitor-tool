...
#region Other Windows hot keys
		...
		UdaController CreateUdaController(int idx)
		{
			...
			// As we now have 100 rather than 10 UDAs and we no longer provide a default set,
			// it no longer makes sense to add magic words for these.
			//Command command = new Command(name, string.Empty, string.Empty, null, false, true);
			Command command = new Command(name, string.Empty, string.Empty, null, false, false);
