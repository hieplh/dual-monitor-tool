...
namespace DMT.Modules.WallpaperChanger
{
	...
	class WallpaperChangerModule : Module
	{
			...
			: base(hotKeyService)
		{
			...
			_providerFactory = new ProviderFactory(_settingsService, _logger);
		...
		/// <summary>
		/// Gets or sets the image format to use for the generated wallpaper
		/// </summary>
		public WallpaperImageFormat.ImageFormat ImageFormat
		{
			get { return (WallpaperImageFormat.ImageFormat)ImageFormatSetting.Value; }
			set { ImageFormatSetting.Value = (int)value; }
		}

		/// <summary>
		/// Gets or sets the image quality to use for the generated wallpaper (only if using JPG)
		/// </summary>
		public int ImageQuality
		{
			get { return ImageQualitySetting.Value; }
			set { ImageQualitySetting.Value = value; }
		}
		...
		IntSetting ImageFormatSetting { get; set; }

		IntSetting ImageQualitySetting { get; set; }
		...
		public override void Start()
		{
			...
			ImageFormatSetting = new IntSetting(_settingsService, ModuleName, "ImageFormat", (int)WallpaperImageFormat.ImageFormat.JPG);
			ImageQualitySetting = new IntSetting(_settingsService, ModuleName, "ImageQuality", 95);
